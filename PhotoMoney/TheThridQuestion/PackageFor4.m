...
Success_now = 0;
Price_now = 0;
Success_bestGlobal = 0;
for n_means = 925:2050
    [Idx,Ctrs,SumD,D] = kmeans(X(:,1:2),n_means,'Replicates',4);
    K_Means_Data = [];
    Sig =[];
    for i = 1:n_means
        ...
        Assign_density = findTopHundredClosestPoint(NewAssignment,Ctrs(i,1),Ctrs(i,2));
...
%         end
        ...
        K_Means_Data = [K_Means_Data;Ctrs(i,1:2),sum(Price2(Idx==i,1)).*(1+0.03).^(sum(Idx==i)-1),sum(Idx==i),sum(Price2(Idx==i,1)).*(1+0.03).^(sum(Idx==i)-1)./sum(Idx==i),Assign_density,dist,Credit,member_density,dist_in];
    ...
    SuccessRate = 1./(1+exp(-ti)).*(1-((K_Means_Data(:,4)-1)./(sqrt(2).*N_max)).^(2.5)).*(1-(K_Means_Data(:,10)./(2.*H_max)).^(2));
    Success = mean(SuccessRate);
    Prices = K_Means_Data(:,3);
    Price_now = sum(Prices);
    Success_now = Success;
    y = 120./Success + 1.*sum(Prices)./(8.*40);
    if y<y_best
        y_best = y;
        n_best = n_means;
    ...
    if Success_now>Success_bestGlobal
        Success_bestGlobal = Success_now;
    end
end